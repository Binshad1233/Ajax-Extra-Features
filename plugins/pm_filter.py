...
async def next_page(bot, query):
    ...
    if int(req) not in [query.from_user.id, 0]:
        return await query.answer("😁 മറ്റുള്ളവർ request ചെയ്ത ക്ലിക്ക് ചെയ്യാതെ നിങ്ങൾക്ക് വേണ്ട സിനിമ request ചെയ്യ്   𝗛𝗲𝘆 𝗙𝗿𝗶𝗲𝗻𝗱,𝗣𝗹𝗲𝗮𝘀𝗲 𝗦𝗲𝗮𝗿𝗰𝗵 𝗬𝗼𝘂𝗿𝘀𝗲𝗹𝗳.", show_alert=True)
    try:
        offset = int(offset)
    except:
        offset = 0
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        buttons = [[
            ...
            InlineKeyboardButton('ᴄᴏɴɴᴇᴄᴛᴇᴛɪᴏɴꜱ', callback_data='coct')
            ],[
...
async def auto_filter(client, msg, spoll=False):
    ...
    if imdb and imdb.get('poster'):
        try:
            await message.reply_photo(photo="https://telegra.ph/file/d8b7b4c713a346dce26ab.jpg", caption=cap[:1024],
                                      reply_markup=InlineKeyboardMarkup(btn))
